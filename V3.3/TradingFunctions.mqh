...
//|                                                                  |
//+------------------------------------------------------------------+
#include <..\Experts\TrendPower\V3.3\Inputs.mqh>
...
public:
   ...
   ulong             lastTicket;
                    ...
                    ~TradingFunctions();
   void              Statistics();
   void              TradingFunctions::GlobalEquityTake();
...
void TradingFunctions::GlobalEquityTake()
  {
   ...
   if(EC.RefEquity==0)
     {
      EC.RefEquity=Equity;
      EC.RefCapital=Capital;//*5.02;
     }
   if(Equity<Capital)
     {
      EC.RefCapital= Capital;
      EC.RefEquity = Equity;
     }
   if(Equity>EC.RefEquity)
     {
      EC.RefEquity=Equity;
      EC.EquityMaxDrop=EC.RefEquity*EQUITY_TAKE_THRESHOLD;//7;

     }
   if(Equity<EC.EquityMaxDrop && Equity>EC.RefCapital)
     {
      if(ENABLE_EQUITY_TAKE)
         GlobalExit(true);
      EC.RefCapital=AccountInfoDouble(ACCOUNT_BALANCE);
      EC.EquityMaxDrop=EC.RefCapital*EQUITY_TAKE_THRESHOLD;
     }
