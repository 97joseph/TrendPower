...
#include <..\Experts\TrendPower\V3.3\TradingFunctions.mqh>
class BaseLine
  {
private:
   string            SYMBOL;
...
int BaseLine::Execute()
  ...
  double Base[];
   ...
   double Rates[];
   CopyClose(SYMBOL,PERIOD_M1,0,3,Rates);
   if(ArraySize(Rates)==0) return FLAT;
   if(ArraySize(Base)==0) return FLAT;
   double pipGap = MathAbs(Rates[1] - Base[1])/point/10;
   
   if (Rates[0] > Base[0] && pipGap >=TREND_POWER_REQUIRE_PIP_GAP) return  UP;
   if (Rates[0] < Base[0] && pipGap >=TREND_POWER_REQUIRE_PIP_GAP) return  DOWN;
